...
rkt_out <- function(rkt){
  tibble(p_value = ifelse(is.na(rkt$sl.corrected)==T, rkt$sl, rkt$sl.corrected), Kendalls_score = rkt$S, Theil_Sens_slope_MK = rkt$B, Kendall_tau=rkt$tau)
}
...
periods <- function(data,
                    ...
                    filter_less_than = NA,
                    max_year=NA){ # filters periods with less than N values (not the length from min to max!)
